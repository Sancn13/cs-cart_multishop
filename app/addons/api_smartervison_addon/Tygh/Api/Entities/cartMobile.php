...
class CartMobile extends ASraEntity
{
    ...
    public function create($params)
    {
        ...
        $this->auth['user_id'] = $user_data['user_id'];

        $this->auth['user_type'] = $user_data['user_type'];

        $this->auth['tax_exempt'] = $user_data['tax_exempt'];

        $this->auth['last_login'] = $user_data['last_login'];

        $this->auth['is_root'] = $user_data['is_root'];

        $status = Response::STATUS_BAD_REQUEST;
        $data = [];

        $dataExtra = fn_get_product_data($params['product_id'], $this->auth, "en", '', true, true, true, false, false, false, false);

        $dataExtra['amount'] = $params['amount'];

        $dataExtra['product_options'] = $params["product_options"];
        ...
        $data_cart_return= [
            ...
            "id"=> key($data["cart_ids"]["products"]),
            "custom_fields"=> [],
            "options"=> $data["cart_ids"]["products"][key($data["cart_ids"]["products"])]["product_options"],
        ];

        return [
            'status' => $status,
            'data'   => [
                "data" => $data_cart_return,
            ],
        ];
    }
